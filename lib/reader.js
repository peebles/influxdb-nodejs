...
class Reader extends QL {
   ...
   * @param {String | Object} key - The key of options, eg: format epoch RP start end limit slimit order offset soffset fill tz
   ...
   *     fill: 0,
   *     // use the specified time zone for binning intervals
  ...
  then(resolve, reject) {
    ...
    if (!this.fullfilledPromise) {
      ...
      const options = internal(this).options;
      let q = this.toSelect();
      if ( this.get( 'tz' ) || this.tz ) {
	q = q + " tz('" + (this.get( 'tz' ) || this.tz) + "')";
      }
   ...
   */
  queue() {
    const queue = internal(this).queue;
    /* istanbul ignore if */
    if (!queue) {
